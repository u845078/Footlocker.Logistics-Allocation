...
namespace Footlocker.Logistics.Allocation.Spreadsheets
{
    public class RingFenceByDestExport : ExportSpreadsheet
    {
        public void WriteData(string div, string store)
        {
            ...
            List<ValidRingFence> ringFenceStoreList = config.db.ValidRingFences.Where(rf => rf.Quantity > 0 && rf.Division == div && rf.Store == store).ToList();

            foreach (ValidRingFence rfStore in ringFenceStoreList)
            {
