...
{
    public class BTSExport : ExportSpreadsheet
    {
        public void WriteData(GridCommand settings, string div, int? year)
        {
            WriteHeaderRecord();
            ...
            {
                bts.StoreLookups = (from a in config.db.StoreBTSDetails
            ...
            foreach (StoreBTSGridExtract rec in list.ToList())
            {
        ...
        public BTSExport(AppConfig config) : base(config)
        {
            maxColumns = 12;
